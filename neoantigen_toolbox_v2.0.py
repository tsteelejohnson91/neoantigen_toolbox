...
def main():
	...
	getmeta = str(sys.argv[3])
	nfiles = 32
	readlen = 50
	peplen = round(readlen/3)
	...
	expr_cutoff((patseq_path+'/tumor_amino_sorted_dups',((peplen-kmer_k+1)/(peplen-8+1))*tumor_depth_constant*(tumor_kmer_depth/2e9)))
	expr_cutoff((patseq_path+'/normal_amino_sorted_dups',((peplen-kmer_k+1)/(peplen-8+1))*normal_depth_constant*(normal_kmer_depth/2e9)))
	...
	if getmeta=='getmeta':
		...
		merge_files((patseq_path,'tumor',32,'_aminodat_metadata'))
